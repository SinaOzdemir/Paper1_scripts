...
data<- readRDS(file = file.path(data_path,"analysis_data_11042022.rds")) %>% 
  ...
  mutate(V201_subject_of_publicity = ifelse(grepl(x=V202_01_Subjects,pattern = "my|us|our|we|I",ignore.case = T)&(nchar(V202_01_Subjects)<=3),
                                            "Self",
                                            V201_subject_of_publicity))



#quick look at the mentions----
data_text<- str_split(string = data$V202_01_Subjects,pattern = ":",simplify = T) %>% as.data.frame()

new_colnames<- gsub("V","_",colnames(data_text)) %>% 
  paste0("other_actor",.)

colnames(data_text)<- new_colnames

data_text$status_id<- data$status_id


data_text_long <- data_text %>%
  pivot_longer(cols = contains("other_actor"),names_to = "actor_order",values_to = "mentioned_actor")


data_text_quick_stat<- data_text_long %>%
  mutate(mentioned_actor = tolower(mentioned_actor)) %>% 
  mutate(mentioned_actor = recode(mentioned_actor, "the eu" = "eu","theeu" = "eu")) %>% 
  mutate(mentioned_actor = trimws(mentioned_actor)) %>% 
  group_by(mentioned_actor) %>% 
  tally() %>% 
  filter(mentioned_actor != "")

data_text_quick_stat %>% 
  slice_max(order_by = n,n = 10) %>% 
  ggplot(aes(x = reorder(as.factor(mentioned_actor),n),y = n)) +
...
sop_max_actor_graph<- actor_type_sop_summary %>% 
  mutate(Actor_type = recode(Actor_type, "High representative and vice president" = "Comissioner")) %>% 
  filter(sop_count == max(sop_count)) %>% 
